/* 				ESKADMAS AYENEW TEFERA - s236174
		...
		CONCURRENT TCP SERVER THAT TRANSFERS FILES
...
int main(int argc, char *argv[])
{
	...
	for(;;)
	{
		...
		{ close(con_req_receiver_skt);		/* close connected socket */
		...
		else
		{
			...
			serve_client(con_req_receiver_skt);	/* serve client */ 
           		exit(1);
		}
	}
	return 0;
...
int serve_client(int con_req_receiver_skt)
{
	...
	{	                       
		...
		if(read_char < 0){
			...
			printf("Connection closed by party on socket %u \n", con_req_receiver_skt);
			close(con_req_receiver_skt);
			break;
		}
		else
		{
			...
			{
				printf("[%s] -- QUIT command received from Client -- \n", prog_name);
				close(con_req_receiver_skt);
				break;
			}
			else if(strncmp(r_buf, "GET ", 4)==0)		/* A command for Client file request */
			{
				int i;		
				...
				if (fd == NULL)
				{
					...
					if (send(con_req_receiver_skt, "-ERR\r\n", 6, 0) != 6)
	    					printf("[%s] -- error in sending response \n", prog_name);
					continue;
				}
				else
				{
					...
					if (send(con_req_receiver_skt, t_buf, sent_char + 13, 0) != (sent_char + 13))/* Send the first packet */
					{
			...
			else	
			{
				...
				if (send(con_req_receiver_skt, "-ERR\r\n", 6, 0) != 6)
	    				printf("[%s] -- error in sending a message\n", prog_name);
