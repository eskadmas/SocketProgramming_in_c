...
int serve_client(int con_req_receiver_skt)
{
	...
	struct stat 		statbuf;		/* stat syscall is used to obtain the timestamp of the last file modification */
	uint32_t 		filesize;
	uint32_t 		lastmod;
	uint16_t		lport_n, lport_h;	/* The port where the server listens(net/host byte order response) */
	...
	{               
		...
		else
		{
			...
			else if(strncmp(r_buf, "GET ", 4)==0)		/* A command from Client to request a file */
			{
				...
				i -=4;
				char *filename;
				filename = (char *) malloc(sizeof(char) * (i+1)); // i+1 is for null character
